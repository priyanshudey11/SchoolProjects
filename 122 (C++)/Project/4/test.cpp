...
TreeNode* insertNode(TreeNode* node, string val) 
{
 ...
 // If the given value is less than the value at the current node, insert it in 
the left subtree
 if (val < node->value) 
 {
 node->left = insertNode(node->left, val);
 } 
 // If the given value is greater than or equal to the value at the current 
node, insert it in the right subtree
 ...
 // If the balance factor is greater than 1 and the value is less than the value
in the left child, perform a right rotation
 if (node->balanceFactor > 1 && val < node->left->value) 
 {
 return rightRotate(node);
 }
 // If the balance factor is less than -1 and the value is greater than the 
value in the right child, perform a left rotation
 ...
 // If the balance factor is greater than 1 and the value is greater than the 
value in the left child, perform a left rotation on the left child followed by a 
right rotation on the current node
 ...
 // If the balance factor is less than -1 and the value is less than the value 
in the right child, perform a right rotation on the right child followed by a left 
...
// This function performs an inorder traversal of the binary search tree and prints
out the values of its nodes in ascending order.
...
// This function prints out the values, left child, right child, height, and 
balance factor of each node in the binary search tree using an inorder traversal.
void printTree(TreeNode* node)
{
 ...
 // Traverse the left 
 printTree(node->left);
 // Print the value of the current node followed by its left and right 
child, height and balance factor
 cout 
 <<setw(10)<< node->value << "\t"
...
// This is the main function that reads in words from a file specified by the 
user,inserts them into a binary search tree, and then prints out the tree in a 
formatted table.
int main()
{
