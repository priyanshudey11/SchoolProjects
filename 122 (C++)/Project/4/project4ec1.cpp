...
TreeNode* createNode(string val) 
{
    ...
    // initialize the multiplicity to 0 since this is the first occurrence of this value
    node->multiplicity = 0;
    return node;
}
...
TreeNode* rightRotate(TreeNode* y) 
{
    ...
    TreeNode* z = x->right;
    // Make y the right child of x change
    x->right = y; 
...
// If the current node is NULL, create a new node with the given value and return it
    ...
    //Updates the multiplicity for the node and returns it
    else // val == node->value
    {
        node->multiplicity++;
        return node;
...
// This function prints the values of a binary search tree in order, taking into account the multiplicity of each value.
void printInorderWithMultiplicity(TreeNode* node) 
{
    // Check if the current node is NULL (i.e., the end of the tree).
    if (node != NULL) 
    {
        // Traverse the left subtree recursively.
        ...
        // Print the value of the current node.
        cout << node->value;
        // Print a newline character to separate values.
        cout << endl;
        // Traverse the right subtree recursively.
